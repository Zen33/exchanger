...
(function($) {
    ...
    $.fn.exchanger = function(options) {
        ...
        var defaults = {
            ...
            exchange = function($oriItem, $aimItem, data, mark, curData, $self) {
        ...
        if (['getData', 'get', 'data'].indexOf(options) !== -1 && this.length === 1) {
            return this.data('curData');
        } else {
            return this.each(function() {
                var $self = $(this),
                    ...
                    init = function() {
                        ...
                        if (typeof $self.attr('id') !== 'undefined' && $self.attr('id') !== false) {
                            namespace = self.getAttribute('id');
                        } else {
                            namespace = 'exchanger_' + (new Date()).valueOf();
                            self.setAttribute('id', namespace);
                        }
                        ...
                        if (opts.hotKeys) {
                            var hotKeys = {
                                ...
                                $nearbyEl = null;
                            $(document).off('keydown.' + namespace + ' keyup.' + namespace + ' mousedown.' + namespace).on('keydown.' + namespace + ' keyup.' + namespace + ' mousedown.' + namespace, function(e) {
                                if (e.type === 'mousedown') {
                        ...
                        $self.off('click.' + namespace + ' dblclick.' + namespace, 'li.left, li.right').on('click.' + namespace + ' dblclick.' + namespace, 'li.left, li.right', function(e) { // 右侧双击不可全选，左右两侧不可同时selected
                            var itemClass = e.target.className;
                            if (e.type === 'click') {
                                if (itemClass.indexOf('left') !== -1) {
                                    $(this).toggleClass('selected');
                                    $('.exchanger .leftSection', document).not($(this).closest('.leftSection')).find('li').removeClass('selected');
                        ...
                        $self.off('click.' + namespace, 'button').on('click.' + namespace, 'button', function(e) {
                            var btnClass = e.target.className,
                                cases = {
                                    'right': function() {
                                        exchange($leftSection, $rightSection, opts.data, opts.mark, curData, $self);
                                    },
                                    'left': function() {
                                        exchange($rightSection, $leftSection, opts.data, opts.mark, curData, $self);
                                    },
                                    'up': function() {
                ...
                if (['reset', 'redo'].indexOf(options) !== -1) {
